...
const LinearLoading = ({ setUserValid }) => {
	...
	useEffect(() => {
		if(progress >= 100 && auth === false){
			...
			alert('There is some problem while loading resources for you. Check your Internet connection, and try again later.');
			setProgress(0);
			setJump(2);
		}
	...
	useEffect(() => {
		const timer = setInterval(() => {
			...
			setProgress((prevProgress) => {
				return (prevProgress >= 100 ? 100 : (prevProgress + jump)>=100? 100 : (prevProgress + jump));
			});
